...
os.environ['PYTORCH_TRANSFORMERS_CACHE'] = "/media/huy/New Volume/khoiai/transformers"
...
parser.add_argument('--stop_after', type=int, default=2)
...
train_df = pd.read_csv("./pretrain/data/train_prep.csv")
...
if args.ignore_neutral or args.use_only not in ["all","neutral"]:
    ...
    #print(train_df.head())
X_train, X_pos_train = np.load(f"./pretrain/data/X_{args.model}.npy"),np.load(f"./pretrain/data/X_positions_{args.model}.npy")
#X_train,X_pos_train = shuffle(X_train,X_pos_train, random_state=42)
#X_train = X_train[:20000]
#X_pos_train = X_pos_train[:20000]
...
for fold, (train_idx, val_idx) in enumerate(splits):
    ...
    train_idx = shuffle(train_idx,random_state=42)[:100000]
    val_idx = shuffle(val_idx,random_state=42)[:5000]
    ...
    frozen = True
#    model.load_state_dict(torch.load("models/pre_{}_{}.bin".format(args.model, fold)))
    for epoch in tq:
        ...
        model.train()
        for i, items in pbar:
            x_batch, x_start_batch, x_end_batch  = items
            if i == 0:
                print(x_batch[:,1])
        ...
        torch.save(model.state_dict(),"models/pre_{}_{}.bin".format(args.model, fold))
